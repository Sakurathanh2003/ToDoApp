...
class CalendarPickerFooterView: UIView {
    ...
    private func updateNextButton() {
        if let baseDate {
            let currentDay = Date()
            let isNextMonthFuture = baseDate.year > currentDay.year || (baseDate.month > currentDay.month - 1 && baseDate.year == currentDay.year)
            nextMonthButton.isUserInteractionEnabled = !isNextMonthFuture
            nextMonthButton.isHidden = isNextMonthFuture
        }
    }
}
